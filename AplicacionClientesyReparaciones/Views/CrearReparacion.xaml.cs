...
namespace AplicacionClientesyReparaciones.Views
{
    public partial class CrearReparacion : UserControl
    {
        ...
        private async void Guardar_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (string.IsNullOrWhiteSpace(Telefono1TextBox.Text))
            {
                MessageBox.Show("El campo 'Móvil 1' es obligatorio.", "Validación", MessageBoxButton.OK, MessageBoxImage.Warning);
                return;
            }

            if (!long.TryParse(Telefono1TextBox.Text, out var telefono1))
            {
                MessageBox.Show("El campo 'Móvil 1' debe contener solo números.", "Validación", MessageBoxButton.OK, MessageBoxImage.Warning);
                return;
            }

            if (!FechaEntregaDatePicker.SelectedDate.HasValue)
            {
                MessageBox.Show("Debe seleccionar una fecha de entrega.", "Validación", MessageBoxButton.OK, MessageBoxImage.Warning);
                return;
            }

            if (string.IsNullOrWhiteSpace(MaterialEntregadoTextBox.Text))
            {
                MessageBox.Show("El campo 'Material entregado' es obligatorio.", "Validación", MessageBoxButton.OK, MessageBoxImage.Warning);
                return;
            }

            if (string.IsNullOrWhiteSpace(DescripcionTextBox.Text))
            {
                MessageBox.Show("El campo 'Descripción' es obligatorio.", "Validación", MessageBoxButton.OK, MessageBoxImage.Warning);
                return;
            }
